...
public class XmlDefinitionIndexer {
  ...
  private void walk(Element e, Path xml, Indexes x, String currentOp){ switch(e.getTagName()){
    case "CCDSEServerOperation" -> { String id=XmlUtil.attr(e,"id"); if(id!=null&&!id.isBlank()){ x.operationToSource().put(id,xml); String ctx=XmlUtil.attr(e,"operationContext"); if(ctx!=null&&!ctx.isBlank()){ x.operationToContext().put(id,ctx); x.contextUsedByOperations().computeIfAbsent(ctx,k->new LinkedHashSet<>()).add(id); } currentOp=id; } }
